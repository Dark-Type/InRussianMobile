...
import com.example.inrussian.components.main.train.TaskBodyChild.AudioConnect
import com.example.inrussian.components.main.train.TaskBodyChild.Empty
import com.example.inrussian.components.main.train.TaskBodyChild.ImageAndSelect
import com.example.inrussian.components.main.train.TaskBodyChild.ImageConnect
import com.example.inrussian.components.main.train.TaskBodyChild.ListenAndSelect
import com.example.inrussian.components.main.train.TaskBodyChild.TextConnect
import com.example.inrussian.components.main.train.TaskBodyChild.TextInput
import com.example.inrussian.components.main.train.TaskBodyChild.TextInputWithVariant
...
sealed interface TrainOutput { data object NavigateBack : TrainOutput
}
...
interface TrainComponent {
    ...
    sealed interface Child { data class CoursesChild(val component: TrainCoursesListComponent) :
        Child
...
class DefaultTrainComponent(
    ...
    private fun createChild(
        config: TrainComponent.Config, ctx: ComponentContext
    ): TrainComponent.Child = when (config) {
        TrainComponent.Config.Courses -> TrainComponent.Child.CoursesChild(
            DefaultTrainCoursesListComponent(
                componentContext = ctx, repository = repository, onNavigateTheme = ::onThemeSelected
            )
        )
        
        is TrainComponent.Config.ThemeTasks -> TrainComponent.Child.ThemeTasksChild(
        ...
        scope.launch {
            ...
            try {
                ...
                _state.value = _state.value.copy(
                    isLoading = false, error = t.message ?: "Load error"
                )
            }
        }
    }
...
sealed interface TaskBodyChild { data class TextConnect(val component: TextConnectTaskComponent) :
    TaskBodyChild
    ...
    data class ImageAndSelect(val component: ImageAndSelectComponent) : TaskBodyChild
...
sealed interface TaskBodyConfig {
    ...
    @Serializable
    @SerialName("ImageSelect")
    data class ImageSelectCfg(val task: TaskBody.ImageAndSelect) : TaskBodyConfig
...
class DefaultThemeTasksComponent(
    ...
    override val childSlot: Value<ChildSlot<TaskBodyConfig, TaskBodyChild>> = childSlot(
        source = navigation,
        ...
        scope.launch {
            state.subscribe { st ->
                ...
                val cfg = when (body) {
                    ...
                    is TaskBody.ImageAndSelect -> TaskBodyConfig.ImageSelectCfg(body)
                    is TaskBody.ConstructSentenceTask, is TaskBody.SelectWordsTask, null -> TaskBodyConfig.EmptyCfg
                }
    ...
    private fun createChild(config: TaskBodyConfig, ctx: ComponentContext): TaskBodyChild =
        when (config) {
            is TaskBodyConfig.TextConnectCfg -> TextConnect(
                TextConnectTaskComponentImpl(
                    ctx, ::onAction, ::onCheckChange, ::toggleButton, config.task
                )
            )
            
            is TaskBodyConfig.ImageConnectCfg -> ImageConnect(
                ImageConnectTaskComponentImpl(
                    ctx, ::onAction, ::onCheckChange, ::toggleButton, config.task
                )
            )
            
            is TaskBodyConfig.AudioConnectCfg -> AudioConnect(
                AudioConnectTaskComponentImpl(
                    ctx, ::onAction, ::onCheckChange, ::toggleButton, config.task
                )
            )
            
            is TaskBodyConfig.TextInputCfg -> TextInput(
                TextInputTaskComponentImpl(
                    ctx, ::onAction, ::onCheckChange, ::toggleButton, config.task
                )
            )
            
            is TaskBodyConfig.TextInputVariantCfg -> TextInputWithVariant(
                TextInputTaskWithVariantComponentImpl(
                    ctx, ::onAction, ::onCheckChange, ::toggleButton, config.task
                )
            )
            
            is TaskBodyConfig.ListenSelectCfg -> ListenAndSelect(
                ListenAndSelectComponentImpl(
                    ctx, ::onAction, ::toggleButton, config.task
                )
            )
            
            TaskBodyConfig.EmptyCfg -> Empty
            is TaskBodyConfig.ImageSelectCfg -> ImageAndSelect(
                ImageAndSelectComponentImpl(
                    ctx, ::onAction, ::toggleButton, config.task
                )
            )
        }
