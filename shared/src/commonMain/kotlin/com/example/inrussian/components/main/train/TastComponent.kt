    ...
    init {
        scope.launch {
            store.labels.collect {
                when (it) {
                    is TrainStore.Label.ChangeTask -> navigation.activate(
                        when (it.task?.taskBody) {
                            is AudioTask -> AudioConnectConfig(it.task.taskBody)
                            is ImageTask -> ImageConnectConfig(it.task.taskBody)
                            is TaskBody.TextInputTask -> TextInputConfig(it.task.taskBody)
                            is TaskBody.TextInputWithVariantTask -> TextInputWithVariantConfig(it.task.taskBody)
                            is TaskBody.TextTask -> TextConnectConfig(it.task.taskBody)
